...
export async function create(name, username, text) {
  ...
  const userData = await userRepository.foundByUsername(username);
...
export async function getAll() {
  ...
  return Promise.all(
    tweets.map(async (tweet) => {
      let { username, name, profileUrl } = await userRepository.foundById(
        tweet.userId
      );
...
export async function findById(id) {
  ...
  const { username, name, profileUrl } = await userRepository.foundById(
    tweet.userId
  );
  return { username, name, profileUrl, ...tweet };
}
